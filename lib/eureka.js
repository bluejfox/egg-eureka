...
const parseRegistryServer = server => {
  ...
  const { hostname, port, path } = url.parse(server);
  return {
    host: hostname,
    port,
    servicePath: path,
...
module.exports = app => {
  ...
  const appName = typeof app.name === 'string' && app.name.trim() !== '' ? app.name : app.config.pkg.name;
  ...
  const instanceInfo = {
    app: `${appName}:${version}`,
    instanceId: `${ipAddr}:${appName}:${version}`,
    hostName,
    ipAddr,
    port: {
      $: port,
